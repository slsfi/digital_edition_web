...
export class TooltipService {
  ...
  getCommentTooltip(id: string): Observable<any> {
      ...
      return this.commentService.getComment(parts[0]).pipe(
        map((data: any) => {
          ...
          if (element) {
            const formatedCommentData = element.innerHTML.replace(/class=\"([a-z A-Z _ 0-9]{1,140})\"/g, 'class=\"teiComment $1\"')
                  .replace(/(teiComment teiComment )/g, 'teiComment ').replace(/(&lt;)/g, '<').replace(/(&gt;)/g, '>');
            return {
              'name': 'Comment',
  ...
  constructPersonTooltipText(tooltip: any, targetElem: HTMLElement) {
    ...
    const yearBornDeceasedString = this.constructYearBornDeceasedString(tooltip.date_born, tooltip.date_deceased);
  ...
  constructYearBornDeceasedString(dateBorn: string, dateDeceased: string) {
    ...
    const yearBorn = (dateBorn !== undefined && dateBorn !== null) ?
                              String(dateBorn).split('-')[0].replace(/^0+/, '').split(' ')[0] : null;
    const yearDeceased = (dateDeceased !== undefined && dateDeceased !== null) ?
                              String(dateDeceased).split('-')[0].replace(/^0+/, '').split(' ')[0] : null;
    const bcIndicatorDeceased = (String(dateDeceased).includes('BC')) ? ' ' + this.BCTranslation : '';
    let bcIndicatorBorn = (String(dateBorn).includes('BC')) ? ' ' + this.BCTranslation : '';
    if (String(dateBorn).includes('BC') && bcIndicatorDeceased === bcIndicatorBorn) {
      // Born and deceased BC, don't add indicator to year born
      bcIndicatorBorn = '';
    }
    let yearBornDeceased = '';
    if (yearBorn !== null && yearDeceased !== null && yearBorn !== 'null' && yearDeceased !== 'null') {
   ...
   */
   getTooltipProperties(targetElem: HTMLElement, ttText: string, pageOrigin = 'page-read') {
    ...
    const contentElem = document.querySelector(pageOrigin + ':not([hidden]) > ion-content > .scroll-content') as HTMLElement;
    if (contentElem !== null) {
    ...
    const tooltipElement: HTMLElement | null = document.querySelector(pageOrigin + ':not([hidden]) div.toolTip');
    ...
    if (positionAboveOrBelowTrigger) {
      ...
      if (initialTTDimensions?.height || 0 <= availableHeight && initialTTDimensions?.width || 0 <= availableWidth) {
        // The tooltip fits without resizing. Calculate position, check for possible overset and adjust.
        x = elemRect.left;
        if (positionAbove) {
          y = elemRect.top - (initialTTDimensions?.height || 0) - primaryToolbarHeight - triggerPaddingY;
        } else {
        ...
        if ((ttNewDimensions?.height || 0) <= availableHeight && (ttNewDimensions?.width || 0) <= availableWidth) {
          // Set new max-width and calculate position. Adjust if overset.
          toolTipMaxWidth = (ttNewDimensions?.width || 0) + 'px';
          x = elemRect.left;
          if (positionAbove) {
            y = elemRect.top - (ttNewDimensions?.height || 0) - primaryToolbarHeight - triggerPaddingY;
          } else {
            y = elemRect.bottom + triggerPaddingY - primaryToolbarHeight;
          }
          // Check if tooltip would be drawn outside the viewport horisontally.
